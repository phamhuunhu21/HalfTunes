...
//  HalfTunes
//
//  Created by Pham Huu Nhu on 12/5/17.
//  Copyright (c) 2017 Pham Huu Nhu. All rights reserved.
//
...
extension SearchViewController: URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        if let url = downloadTask.originalRequest?.url?.absoluteString {
            ...
            if let trackIndex = trackIndexForDownloadTask(downloadTask: downloadTask) {
                DispatchQueue.main.async {
                    self.tableView.reloadRows(at: [IndexPath(row: trackIndex, section: 0)], with: UITableViewRowAnimation.none)
                }
            }
        }
    }
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        ...
        if let downloadUrl = downloadTask.originalRequest?.url?.absoluteString,
            ...
            //let totalSize = ByteCountFormatter.stringFromByteCount(totalBytesExpectedToWrite, countStyle: ByteCountFormatterCountStyle.Binary)
            // 4
